    ...
    ajaxUploadSettings ({ state, commit, dispatch }) {
      if (state.enableAjax) {
        window.$.getJSON('/djeym/ajax-upload-settings-editor/',
          ...
          .done(data => {
            state.widthPanelSettings = data.generalSettings.controls.panel2_70[1].widthPanelEditor
            dispatch('categoryFilters/actionSetFromAjax', data.categories, { root: true })
            commit('tileSources/setFromAjax', data.tileSources, { root: true })
            dispatch('generalSettings/actionSetFromAjax', data.generalSettings, { root: true })
            dispatch('mapControls/actionSetFromAjax', data.mapControls, { root: true })
            dispatch('heatmap/actionSetFromAjax', data.heatmapSettings, { root: true })
            dispatch('loadIndicators/actionSetFromAjax', data.loadIndicators, { root: true })
    ...
    ajaxContextMenu ({ state, commit, dispatch, rootState }) {
      ...
      switch (geoType) {
        case 'heatpoint':
          ...
          dataGeoObj.append('coordinates', JSON.stringify(rootState.dialogCreate.coords))
          break
        case 'placemark':
          ...
          if (editableGeoObject === null) {
            ...
            dataGeoObj.append('coordinates', JSON.stringify(placemark.coordinates))
          }
        ...
        case 'polyline':
          ...
          dataGeoObj.append(
            ...
            typeof route.strokeStyle === 'string'
              ? route.strokeStyle : route.strokeStyle.value)
        ...
        case 'polygon':
          ...
          dataGeoObj.append(
            ...
            typeof territory.strokeStyle === 'string'
              ? territory.strokeStyle : territory.strokeStyle.value)
          ...
          dataGeoObj.append('coordinates', JSON.stringify(territory.coordinates))
      ...
      if (state.enableAjax) {
        window.$.ajax({
          ...
          .done((data) => {
            ...
            switch (geoType) {
              case 'heatpoint':
                dispatch('contextmenuHeatPoint/restoreDefaults', null, { root: true })
                dispatch('ymap/addHeatPoints', data, { root: true })
                break
              case 'placemark':
                dispatch('contextmenuPlacemark/restoreDefaults', null, { root: true })
                if (actionType !== 'delete') {
                  dispatch('ymap/addPlacemarkTypeObjects', data, { root: true })
                }
                break
              case 'polyline':
                dispatch('contextmenuRoute/restoreDefaults', null, { root: true })
                if (actionType !== 'delete') {
                  dispatch('ymap/addPolylineTypeObjects', data, { root: true })
                }
                break
              case 'polygon':
                dispatch('contextmenuTerritory/restoreDefaults', null, { root: true })
                if (actionType !== 'delete') {
