    ...
    <div style="width: 422px; height: 500px; overflow: auto;">
      <v-row dense justify="start" align="center" style="width: 422px;">
        <v-col cols="12" class="pa-0">
          <v-card-subtitle class="py-0 font-italic">{{ `-${$t('message.20')}` }}</v-card-subtitle>
        </v-col>
        <template v-for="item in categoryList">
          <v-col cols="1" :key="`icon-category-${item.id}`">
            <v-icon :color="item.color" class="pos-icon">{{ item.icon }}</v-icon>
          </v-col>
        ...
        <v-col cols="12" class="pa-0">
          <v-card-subtitle class="pb-0 font-italic">{{ `_${$t('message.21')}` }}</v-card-subtitle>
        </v-col>
        <template v-for="item in subcategoryList">
          ...
          <v-col cols="11" :key="`switch-subcategory-${item.id}`">
            <v-switch
              v-model="controls.subcategories"
              multiple
              inset
              :value="item.id.toString()"
              hide-details
...
export default {
  ...
  methods: {
    ...mapMutations('selectingCategories', [
      'setCategory',
      'setSubcategories'
    ])
  },
  created () {
    ...
    this.setSubcategories(this.dataAction.subcategories.map(item => item.toString()))
    // Add Category Lists.
