...
function init() {
    ...
    window.GeocoderMap = new djeymYMaps.Map("djeymYMapGeocoder", {
        center: [0, 0],
        zoom: 3,
    ...
    if (window.djeymControls === false) {
        GeocoderMap.controls.get("searchControl").options.set("provider", "yandex#search");
    }

    // Connect a third-party source of tiles.
    if (typeof window.djeymTile !== "undefined") {
        GeocoderMap.layers.add(new djeymYMaps.Layer(
            window.djeymSource(), {
                projection: djeymYMaps.projection.sphericalMercator
            }));

        if (window.djeymTile.copyrights.length > 0) {
            GeocoderMap.copyrights.add(window.djeymTile.copyrights);
        }
    }

    if (window.djeymAddress.length > 0) {
        GeocoderMap.events.once("boundschange", function () {
            stopSpinIndication();
        });
        ...
        djeymYMaps.geocode(window.djeymAddress, {results: 1}).then(
            function (res) {
                ...
                GeocoderMap.geoObjects.add(firstGeoObject);
                GeocoderMap.setBounds(bounds, {checkZoomRange: true});
            },
            function (err) {
                console.error(err);
                stopSpinIndication();
