...
<template>
  <v-conteiner fluid>
    <v-row align="center" justify="center" style="width:320px;">
      <v-btn
    ...
    <v-row class="pt-5" style="width:320px;">
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('ymap', [
      'setEditableGeoObject'
    ]),
    ...mapActions([
      'ajaxContextMenu'
    ]),
    ...mapActions('dialogCreate', [
      'actionCreateNewGeoObjecte'
    ]),
    // Rules
    rulesLatitude () {
      return [
    ...
    messageErrorAddCategory () {
      return '<div class="djeym-fake-buttons" style="background:' +
        ...
        this.colorButtonsTextTheme + ';"></span></div><br>' + i18n.t('message.105')
    },
    messageErrorAddHeader () {
      return '<div class="djeym-fake-buttons" style="background:' +
        this.colorControlsTheme + ';">' +
        '<span class="mdi mdi-page-layout-header mdi-24px" style="color:' +
        this.colorButtonsTextTheme + ';"></span></div><br>' + i18n.t('message.106')
    },
    // Create Geo-object
    createGeoObject (index) {
      if (index === 0 || index === 3) {
        ...
        if (!helpers.checkLatitude(latitude) || !helpers.checkLongitude(longitude)) {
      ...
      switch (index) {
        case 0: // Create Placemark.
          this.geoObjectDialogShow({
            ...
            actionBtnEdit: null,
            actionBtnSave: () => {
              let isLatitude = helpers.checkLatitude(window.$('#id-djeym-latitude').val())
              let isLongitude = helpers.checkLongitude(window.$('#id-djeym-longitude').val())
        ...
        case 2: // Create Territory.
          ...
          newGeoObjecte.events.add('contextmenu', () => {
            this.actionCreateNewGeoObjecte({
              ...
              showSaveBtn: true,
              storeModuleName: { 1: 'contextmenuRoute', 2: 'contextmenuTerritory' }[index],
