...
def get_size_from_svg(svg):
    ...
    with open(svg.path, mode="rb") as svg_file:
        ...
        result = {"width": int(Decimal(root.get("width"))), "height": int(Decimal(root.get("height")))}
    return result
    ...
    if bool(svg_path):
        ...
        if bool(tag) and tag == "{http://www.w3.org/2000/svg}svg":
            if root.get("width") is not None and root.get("height") is not None:
                ...
                svg_path.close()
                raise ValidationError(_("There are no width and height attributes in the SVG tag."))
        else:
            svg_path.close()
            raise ValidationError(
                _("This is an invalid SVG file. Open this file in the vector editor and try to fix it."),
...
def validate_coordinates(coordinate):
    try:
        ...
        raise ValidationError(_("To determine the coordinates a numeric value is required."))
...
def validate_transparency(coordinate):
    try:
        ...
        raise ValidationError(_("To determine the transparency a numeric value is required."))
    ...
    for form in args:
        for item in list(form):
            if item.errors:
                ...
                detail += "{}: {}<br>".format(item.label, item.errors)
            else:
...
def make_upload_path(instance, filename):
    ...
    return os.path.join(instance.upload_dir, "{}{}".format(uuid.uuid4(), extension))
...
def cleaning_files_pre_save(sender, instance, **kwargs):
    ...
    if old_status.count() > 0:
        for dir_name in dir_name_list:
            ...
            if bool(field) and (
                isinstance(field, FileDescriptor)
                or isinstance(field, ImageFileDescriptor)
                or isinstance(field, ImageSpecField)
            ):
