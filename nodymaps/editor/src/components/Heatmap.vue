          ...
          <v-container fluid class="pa-0">
            ...
            <v-row>
              <v-col cols="12">
                ...
                >
                  <v-icon color="grey darken-4" class="mr-2"
                    >mdi-restart</v-icon
                  >
      ...
      <template v-else>
        <v-simple-table>
          <template #default>
            <tbody>
              <tr
                v-for="(val, key, index) in controls.panel2"
                :key="`control-${key}`"
              >
                <td :class="key !== 'dissipating' ? 'pb-1' : 'pb-0'">
                  ...
                  <v-card-actions class="mb-0">
                    ...
                    <template
                      v-else-if="key === 'opacity' || key === 'intensity'"
                    >
  ...
  methods: {
    ...
    recolor(target) {
      ...
      this.controlsDialogShow({
        title: `${this.$t("message.48")} - ${
          target[0].toUpperCase() + target.slice(1, 5) + " " + numColor
        }`,
        text: "",
        cancelBtn: true,
        saveBtn: true,
