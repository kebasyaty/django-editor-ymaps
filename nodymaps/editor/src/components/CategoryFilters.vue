        ...
        <v-expansion-panels v-model="panel" focusable>
          <v-expansion-panel>
            ...
            <v-expansion-panel-content>
              ...
              <div class="subtitle-1 mt-5 text-center">{{ $t('message.80') }}</div>
              <v-divider></v-divider>
              <div
              ...
              <v-divider></v-divider>
              <div class="caption text-center">{{ `(${$t('message.144')})` }}</div>
              ...
              <div class="subtitle-1 mt-5 text-center">{{ $t('message.139') }}</div>
              <v-divider></v-divider>
              <div class="caption text-center">{{ `(${$t('message.20')}, ${$t('message.21')})` }}</div>
              ...
              <div class="subtitle-1 mt-5 text-center">{{ $t('message.145') }}</div>
              <v-divider></v-divider>
    ...
    <v-tabs-items v-model="filtersTab">
      ...
      >
        <template v-if="index === 0">
          ...
          >{{ geoTypeNameMarker ? geoTypeNameMarker : $t('message.15') }}</v-card-title>
        </template>
        <template v-if="index === 1">
          <v-card-title
            ...
            class="title pt-1 pb-0"
            :class="centerGeoTypes ? 'justify-center' : ''"
          >{{ geoTypeNameRoute ? geoTypeNameRoute : $t('message.16') }}</v-card-title>
        </template>
        <template v-if="index === 2">
          <v-card-title
            ...
            class="title pt-1 pb-0"
            :class="centerGeoTypes ? 'justify-center' : ''"
          >{{ geoTypeNameTerritory ? geoTypeNameTerritory : $t('message.17') }}</v-card-title>
        </template>
        ...
        <v-container fluid class="pt-1">
          ...
          >
            <template v-if="index2 === 0">
              ...
              >{{ filter.length && !hideGroupNames ? groupNameCategories ? groupNameCategories : $t('message.20') : '' }}</v-card-subtitle>
            </template>
            <template v-if="index2 === 1">
              <v-card-subtitle
                class="font-italic px-0 pb-1"
                :class="index2 ? 'pt-1' : 'pt-0'"
              >{{ filter.length && !hideGroupNames ? groupNameSubcategories ? groupNameSubcategories : $t('message.21') : '' }}</v-card-subtitle>
            </template>
            ...
            >
              <v-list-item
                ...
                :ripple="effectRipple"
                class="mb-1"
                @click="[control.isActive = !control.isActive, actionFiltering({id: control.id, modelKey: modelKey})]"
              >
...
export default {
  ...
  methods: {
    ...
    rulesReplacingCategoryIcons () {
      return [
        iconName => !!iconName || this.$t('message.53')
      ]
    },
