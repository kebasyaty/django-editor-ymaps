/*
 * Editor Yandex Maps.
 * Creating thumbnails for `Placemark.image_geo_object` field.
...
$(document).ready(function () {
    ...
    if (fileUpload !== undefined) {
      ...
      reader.onload = function (event) {
        ...
        tempImg.onload = function () {
          ...
          const canvas = document.getElementById("resize_image_temp_canvas");
  ...
  /* Image size correction before adding to the `Placemark.image_geo_object`.
    (Коррекция размера изображения перед добавлением в `Placemark.image_geo_object`.) */
  try {
        ...
        if (dialogName === "image") {
          ...
          dialogDefinition.onOk = function (event) {
            ...
            if (tmpWidth > MAX_WIDTH) {
              height.setValue(
                Math.floor(+height.getValue() * (MAX_WIDTH / tmpWidth)),
              );
    ...
    runResizeImage = function () {
      ...
      $(document).on("click", "a.cke_dialog_tab_selected", function () {
        ...
        const buttonLoadOnServer = commonParent.find(
          "a.cke_dialog_ui_fileButton",
        );
        const parentOfButton = buttonLoadOnServer.parent();
        const iframeContents = commonParent
          .find("td.cke_dialog_ui_vbox_child iframe")
          .contents();
        ...
        parentOfButton
          .parent()
          .after(
            '<tr class="cke_info_upload_image"><td>' +
              "<div>( " +
              window.gettext("Image size will be optimized to 966 pixels.") +
              " )</div></td></tr>",
          );
        ...
        controlAgent.on("click", function () {
          ...
          const fileUpload = formOfUpload.find('input[name="upload"]')[0]
            .files[0];
