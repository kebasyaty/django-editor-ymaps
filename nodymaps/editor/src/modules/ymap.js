...
export default {
  ...
  actions: {
      ...
      if (state.editableGeoObject !== null) {
        state.Map.setBounds(state.editableGeoObject.geometry.getBounds(), {
          ...
          .then(() => {
            ...
            commit('modals/alertSnackbarShow', i18n.t('message.129'), { root: true })
          }, this)
      }
    },
    // Create a map.
    createYMap ({ state, commit, dispatch, rootState }) {
      ...
      const isTypeSelector = mapControls.activeControls.includes('typeSelector')
      const activeControls = mapControls.activeControls.filter((title) => title !== 'typeSelector')
      ...
      const randomTileUrl = (currentTile !== null)
        // eslint-disable-next-line no-new-func
        ? new Function('return ' + currentTile.randomTileUrl) : null

      const heatmapControls = rootState.heatmap.controls

      // UPLOAD INDICATOR --------------------------------------------------------------------------
      let spinTimers = []
      function waitLoadContent () { // wait upload content to Balloon
        spinTimers[3] = setTimeout(() => {
          ...
          const $images = window.$('ymaps:regex(class, .*-balloon__content) img')
          let imgLoaded = false
          let counter = 0
      ...
      const customBalloonContentLayout = window.djeymYMaps.templateLayoutFactory.createClass(
      ...
      const customBalloonContentLayoutForEditable = window.djeymYMaps.templateLayoutFactory.createClass(
      ...
      if (isTypeSelector) {
        ...
        if (isRoundTheme) {
          ...
          typeSelector = new window.djeymYMaps.control.TypeSelector()
          typeSelector.options.set('panoramasItemMode',
            generalSettings.controls.panel1_69[1].isActive ? 'ifMercator' : 'off')
        }
        ...
        if (currentTile !== null) {
          let currentTilLayer = function () {
            let layer = new window.djeymYMaps.Layer(
              randomTileUrl(), { projection: window.djeymYMaps.projection.sphericalMercator }
            )
            // Copyright.
            ...
            layer.getZoomRange = function () {
              return window.djeymYMaps.vow.resolve([currentTile.minZoom, currentTile.maxZoom])
            }
            return layer
          }
          ...
          window.djeymYMaps.mapType.storage.add('tile#current', currentTilMapType)
        ...
        if (currentTile !== null) {
          ...
          Map.layers.add(new window.djeymYMaps.Layer(
            randomTileUrl(), { projection: window.djeymYMaps.projection.sphericalMercator }
          ))
          // Add Copyrights.
      ...
      if (isRoundTheme) {
        if (activeControls.includes('geolocationControl')) {
          let geolocationControl = new window.djeymYMaps.control.GeolocationControl({
            options: {
        ...
        if (activeControls.includes('searchControl')) {
          ...
          customSearchControl.events.add('press', () => {
            window.$('ymaps:regex(class, ymaps-.+-float-button-icon_icon_magnifier)')
              .trigger('click')
          })
        ...
        if (activeControls.includes('fullscreenControl')) {
          let fullscreenControl = new window.djeymYMaps.control.FullscreenControl({
            data: {
              iconType: 'expand'
            },
            options: {
        ...
        if (activeControls.includes('searchControl') && isSearchByOrganization) {
      ...
      Map.events.add('click', (mapEvent) => {
        ...
        commit('modals/controlsDialogShow', {
      ...
      const clusterRadius = parseInt(Math.min.apply(null, state.cluster.size) / 2)
      const objMngPlacemarkOptions = {
        ...
        clusterIconContentLayout: generalSettings.controls.panel1_69[3].isActive
          ? customLayoutForClusterIcon : null,
        ...
        clusterBalloonContentLayout: generalSettings.controls.panel1_69[2].layout,
        clusterIcons: [{
      ...
      state.globalObjMngPlacemark = new window.djeymYMaps.ObjectManager(objMngPlacemarkOptions)

      // Create a manager for Polylines.
      state.globalObjMngPolyline = new window.djeymYMaps.ObjectManager(geoObjectBalloonOptions)

      // Create a manager for Polygons.
      state.globalObjMngPolygon = new window.djeymYMaps.ObjectManager(geoObjectBalloonOptions)
      ...
      state.globalObjMngPlacemark.clusters.events.add('click', (event) => {
        ...
        for (let idx = 0; idx < countObjs; idx++) {
          state.globalObjMngPlacemark.clusters.balloon.setData(clearContentGeoObject(geoObjects[idx]))
        }
        ...
        window.$.get('/djeym/ajax-balloon-content/', {
          ...
          window.$('ymaps:regex(class, .*-cluster-tabs__menu-item.*)').eq(0).trigger('click')
      ...
      const messageErrorAddCategory = () => {
        return '<div class="djeym-fake-buttons" style="background:' +
          ...
          ';"></span></div><br>' + i18n.t('message.105')
      }
      const messageErrorAddHeader = () => {
        return '<div class="djeym-fake-buttons" style="background:' +
          ...
          rootState.generalSettings.colorButtonsTextTheme +
          ';"></span></div><br>' + i18n.t('message.106')
      }
      ...
      function transferToContextMenu (geoObject) {
        ...
        window.$.get('/djeym/ajax-balloon-content/',
          { objID: pk, objType: geoObjectType, isPresets: 'False' }
        ).done(data => {
          ...
          } else if (geoObjectType === 'LineString' || geoObjectType === 'Polygon') {
          ...
          let contextMenu = (payload) => {
            ...
            commit('modals/geoObjectDialogShow', {
              title: i18n.t(`message.${{ Point: 100, LineString: 109, Polygon: 111 }[payload.geoType]}`),
              editBtn: true,
              saveBtn: true,
              cancelBtn: true,
              deleteBtn: true,
              actionBtnEdit: () => commit('modals/geoObjectDialogClose', null, { root: true }),
              actionBtnSave: () => {
                ...
                if (payload.geoType === 'Point') {
                  ...
                  errorHeader = rootState.contextmenuPlacemark.header.length === 0
                } else if (payload.geoType === 'LineString') {
                  errorCategory = !rootState.contextmenuRoute.category
                  errorHeader = rootState.contextmenuRoute.header.length === 0
                } else if (payload.geoType === 'Polygon') {
                  errorCategory = !rootState.contextmenuTerritory.category
                  errorHeader = rootState.contextmenuTerritory.header.length === 0
                }
                if (errorCategory) {
