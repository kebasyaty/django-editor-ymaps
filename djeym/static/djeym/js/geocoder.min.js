...
function init() {
    "use strict";
    let html = "<div id=\"djeymModalLockGeocoder\"><div id=\"djeymLoadIndicatorGeocoder\"></div></div>";
    document.getElementById("djeymYMapGeocoder").innerHTML = html;

    function stopSpinIndication() {
        let modalLock = document.getElementById("djeymModalLockGeocoder");
        setTimeout(function () {
    ...
    if (typeof window.djeymTile !== "undefined") {
        Map.layers.add(new djeymYMaps.Layer(window.djeymSource(), {projection: djeymYMaps.projection.sphericalMercator}));
    ...
    if (window.djeymAddress.length > 0) {
        ...
        djeymYMaps.geocode(window.djeymAddress, {results: 1}).then(function (res) {
