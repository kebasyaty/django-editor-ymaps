      ...
      >
        <v-container fluid class="pa-0" :style="`min-height: 100%; background-color: ${tinting};`">
          ...
          <v-tabs
            ...
            :color="colorControls"
            :style="isHideTabs ? 'position: relative; top: -45px; z-index: -10;' : ''"
          >
          ...
          <v-tabs-items
            v-model="tab"
            :style="`background-color: transparent;${isHideTabs ? 'top: -45px;' : '' }`"
          >
            <v-tab-item v-for="(step, index) in 3" :key="`item-${step}`" :value="`tab-${index}`">
              <template v-if="index === 0">
                ...
                >{{ geoTypeNameMarker ? geoTypeNameMarker : $t('message.1') }}</v-card-title>
              </template>
              <template v-if="index === 1">
                <v-card-title
                  ...
                  class="title pt-3 pb-0 px-3"
                  :class="centerGeoTypes ? 'justify-center' : ''"
                >{{ geoTypeNameRoute ? geoTypeNameRoute : $t('message.2') }}</v-card-title>
              </template>
              <template v-if="index === 2">
                <v-card-title
                  ...
                  class="title pt-3 pb-0 px-3"
                  :class="centerGeoTypes ? 'justify-center' : ''"
                >{{ geoTypeNameTerritory ? geoTypeNameTerritory : $t('message.3') }}</v-card-title>
              </template>
              ...
              <v-container fluid class="pt-0">
                ...
                >
                  <template v-if="index2 === 0">
                    ...
                    >{{ filter.length && !hideGroupNames ? groupNameCategories ? groupNameCategories : $t('message.4') : '' }}</v-card-subtitle>
                  </template>
                  <template v-if="index2 === 1">
                    <v-card-subtitle
                      class="font-italic px-0 pb-1"
                      :class="index2 ? 'pt-1' : 'pt-0'"
                    >{{ filter.length && !hideGroupNames ? groupNameSubcategories ? groupNameSubcategories : $t('message.5') : '' }}</v-card-subtitle>
                  </template>
                  ...
                  >
                    <v-list-item
                      ...
                      :ripple="effectRipple"
                      class="mb-1"
                      @click="[control.isActive = !control.isActive, filtering({id: control.id, modelKey: modelKey})]"
                    >
  ...
  methods: {
      ...
      if (this.enableAjax) {
        window.$.getJSON('/djeym/ajax-upload-settings-front/',
          ...
          .done(data => {
            ...
            this.$vuetify.theme.dark = { light: false, dark: true }[this.themeType]
            ...
            // Start create Map.
            window.djeymYMaps.ready()
              .then(() => this.initMap(data))
          })
    ...
    saveCustomMarker () {
      ...
      if (!customMarkerCategory) {
        ...
        this.textAlert = `${this.$t('message.11')} --> ${this.$t('message.12')}`
        this.showAlert = true
        return
      }
    ...
    initMap (data) {
      ...
      const activeControls = data.activeControls.filter((title) => title !== 'typeSelector')
      const isRoundTheme = data.isRoundTheme
      const heatmap = data.heatmap

      // UPLOAD INDICATOR
      let spinTimers = []
      const waitLoadContent = () => { // wait upload content to Balloon
      ...
      const customLayoutForClusterIcon = window.djeymYMaps.templateLayoutFactory.createClass(
        ...
        data.colorBackgroundCountObjects + ';color:' +
        data.textColorCountObjects +
        ';">$[properties.geoObjects.length]</span></div>'
      )

      // CREATE A MAP
      const Map = new window.djeymYMaps.Map('djeymYMapsID', {
      ...
      if (isRoundTheme) {
        ...
        if (activeControls.includes('searchControl') && data.isSearchByOrganization) {
      ...
      const clusterRadius = parseInt(Math.min.apply(null, data.cluster.size) / 2)
      ...
      this.globalObjMngPlacemark = new window.djeymYMaps.ObjectManager(objMngPlacemarkOptions)

      // Create a manager for Polylines.
      this.globalObjMngPolyline = new window.djeymYMaps.ObjectManager(geoObjectBalloonOptions)

      // Create a manager for Polygons.
      this.globalObjMngPolygon = new window.djeymYMaps.ObjectManager(geoObjectBalloonOptions)
      ...
      this.globalObjMngPlacemark.clusters.events.add('click', (event) => {
        ...
        for (let idx = 0; idx < countObjs; idx++) {
          this.globalObjMngPlacemark.clusters.balloon.setData(clearContentGeoObject(geoObjects[idx]))
        }
              ...
              if (data.length > 0) {
                ...
                setTimeout(() => {
                  ...
                  setTimeout(() => {
                    ...
                    window.$(document).on('click', '#djeym-open-panel', () => {
                      this.openPanel = true
                      this.isPermanentPanel = true
                    })
                    window.$(document).on('click', '#djeym-add-marker', () => {
                      if (!this.openForm) {
                        this.Map.balloon.close()
                        ...
                        this.customPlacemark = new window.djeymYMaps.Placemark(Map.getCenter(), {
                          hintContent: '',
                          balloonContent: ''
                        }, {
