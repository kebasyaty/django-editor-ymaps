...
export default {
  ...
  actions: {
    ...
    actionSetFromAjax ({ state, dispatch }, obj) {
      ...
      state.disabledBtnDelImgBgPanelFront = !state.controls.panel3_71[3].imgBgPanelFront
    },
    // Update type theme.
    ...
    actionPalette ({ state, commit, rootState }) {
      ...
      switch (target) {
        ...
        case 'tintingPanelFront':
          if (hexColor.length === 7) { hexColor += 'FF' }
          break
        default:
    ...
    actionAjaxUpdate ({ state, commit, dispatch, rootState }) {
      if (rootState.enableAjax) {
        ...
        if ((!isFinite(widthPanelEditor) || widthPanelEditor < 300) ||
          (!isFinite(widthPanelFront) || widthPanelFront < 260)) {
          commit('modals/globalProgressBarShow', false, { root: true })
        ...
        fd.append('cluster_icon_content_bg_color', state.colorBackgroundCountObjects)
        fd.append('cluster_icon_content_txt_color', state.textColorCountObjects)
        ...
        fd.append('img_bg_panel_front_b64', /base64/.test(imgBgPanelFront) ? imgBgPanelFront : '')
        fd.append('tinting_panel_front', state.tintingPanelFront)
        fd.append('width_map_front', state.controls.panel3_71[4].widthMapFront)
        fd.append('height_map_front', state.controls.panel3_71[5].heightMapFront)
        ...
        window.$.ajax({
          ...
          .done(() => {
            if (state.dataActionSaveUpdate.reloadPage) {
              ...
              commit('setDisabledBtnDelImgBgPanelFront',
                !state.controls.panel3_71[3].imgBgPanelFront)
              commit('modals/globalProgressBarShow', false, { root: true })
              commit('setMapSettingsDrawer', true, { root: true })
            }
